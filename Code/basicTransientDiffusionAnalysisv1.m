...
%                         free, 3 = directed, 0 = immobile, NaN = unclassified.
...
%                          confined or immobile (also calculated for other classifications).
...
%
%REMARKS: 
%(1) plotting option may not always work
%(2) need to make sure that confRad (diffusion radius) is consistent for
%free diffusion type. 
%TN 20200129
%
    ...
    endPoint  = partClassMSS(k,2);
                ...
                if asymFlag ==1
                    ...
                    elseif pointClassMSS(k) == 0
                        ...
                        % confRadTmp(k) = NaN; % TN20200302 to uniform diffusion radius output
                        [confRadTmp(k),centerTmp(k,:)] = estimConfRad(tracks(iTrack,8*(startPoint-1)+1:8*endPoint),probDim,confRadMin);
                    end
                    partClassMSS(k,3) = pointClassMSS(k);
                end
