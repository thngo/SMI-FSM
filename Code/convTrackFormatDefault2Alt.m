...
% Tra Ngo, September 2020 - add handling to aggregState field.
%
...
%if exist)
aggregField = isfield(compTracks,'aggregState');
...
for iTrack = 1 : numTracks
    ...
    %TN 20200924: add handling of aggregState
    if aggregField
        aggregStateMat = compTracks(iTrack).aggregState;
        aggregStateMatAlt = aggregStateMat';
    end
    ...
    for iEventTmp = 1 : numMSEvents
        ...
        if event2do(iEventTmp)
            ...
            % TN 20200924: update aggregState matrix
            if aggregField
               aggregStateMatAlt((doubleFreq+1)*eventTime:end,numSegmentsAlt) = aggregStateMatAlt(...
                    (doubleFreq+1)*eventTime:end,originalSegment);
                aggregStateMatAlt(1:(doubleFreq+1)*eventTime-1,numSegmentsAlt) = NaN;
                aggregStateMatAlt((doubleFreq+1)*eventTime:end,originalSegment) = NaN;
            end
            ...
            switch eventType
                ...
                case 2 %if it's a merge
                    ...
                    if doubleFreq
                        ...
                        % TN 20200924: save aggregState in compTracksAlt output
                        if aggregField
                            aggregStateMatAlt((doubleFreq+1)*eventTime-1,mergingSegment) = ...
                                aggregStateMatAlt((doubleFreq+1)*eventTime-2,mergingSegment);
                        end
    ...
    if aggregField %TN 20200924: add handling of aggregState
        aggregStateMatAlt = aggregStateMatAlt';
    end
    ...
    % TN 20200924: save aggregState in compTracksAlt output
    if aggregField
        compTracksAlt(iTrack).aggregState = aggregStateMatAlt;
    end
