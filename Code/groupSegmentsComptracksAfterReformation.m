...
% Add handling to input field 'aggregState'. Tra Ngo, September 2020.
%
...
%if exist)
aggregField = isfield(tracksIn,'aggregState');
%% gropup segments
...
for iTrack = 1: length(compTracks)
    ...
    if lengthSeqOfEvents > 2
        ...
        for indexGroup=1:length(compTrackSegCell)
            ...
            if aggregField
                tracksReform( iTrackCurr).aggregState = aggregStateMat(segmentGroup,:);
            end
            ...
            if aggregField
                tracksReform( iTrackCurr).aggregState = tracksReform( iTrackCurr).aggregState(:,timeDiffStart+1:end-timeDiffEnd);
            end
